...
class BikesharingQsimModule extends com.google.inject.AbstractModule {
	...
	Collection<AbstractQSimPlugin> provideQSimPlugins(TransitConfigGroup transitConfigGroup, NetworkConfigGroup networkConfigGroup, Config config) {
		...
		plugins.add(new BikesharingPopulationPlugin(config));
		return plugins;
	}
	
	private static class BikesharingPopulationPlugin extends AbstractQSimPlugin {
		public BikesharingPopulationPlugin(Config config) { super(config); }
		@Override 
